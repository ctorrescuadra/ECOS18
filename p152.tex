...
\hypersetup{ % Please edit following fields
       ...
       pdftitle    = {The Flow-Process Table. Part I: Fundamentals},%
...
The problem of exergy costs allocation was formulated by Valero et al. in 1986, \cite{Valero1986,ECT93} as follows: Given a system whose boundaries and level of aggregation that specifies the processes, which constitute it, have been defined. How to obtain the costs of all flows (energy carriers) that becomes interrelated in this structure?
...
\Cref{tab1} shows the definition of the fuel and product streams of each process in the case of TGAS plant. In this example,the net power $E_7$, and the exergy required to evaporate the water $E_8$ are the final products of the system, and the exergy $E_5$ provided by the natural gas is the total fuel. The purpose of the compressor is to increase the pressure (and the exergy) of the air from the environment to $E_1$ using the mechanical work produced by the turbine. The aim of the combustor is to increase the exergy of the air flow from $E_1$ that exits from the compressor to $E_2$ and the exergy of natural gas $E_5$ is consumed as fuel.
...
\begin{table}[H]
	...
	\label{tab3}%
\end{table}%
Let \vm{F} and \vm{P} to be $n\times1$ vectors, whose elements are the fuel and product of the processes, and \vm{E} a $m\times1$ vector, whose elements are the exergy of the flows.
...
Let \mbt{R} to be a $n \times n$ matrix, called \emph{waste-process table}, whose elements $r_{iu}$ represent the exergy of waste flow $i\in\mathcal{R}$ generated by the productive component $u\in\mathcal{V}_P$. This matrix verify the following properties:
\begin{align}
...
\end{nomenclatura}
\begin{nomenclatura}[2.5em]{Subscripts, superscripts and accents}
